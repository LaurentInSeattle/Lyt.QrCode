...
public static class BitArrayExtensions
{
    ...
    /// The bit at index <paramref name="index"/> + <paramref name="bitCount"/> - 1 becomes the least significant bit.
    ...
    public static uint ExtractBits(this BitArray bitArray, int index, int bitCount)
    {
        if (bitCount < 0 || bitCount > 31)
        {
            throw new ArgumentOutOfRangeException(nameof(bitCount), "'bitCount' out of range");
        }

        if (index < 0 || index + bitCount > bitArray.Length)
        {
