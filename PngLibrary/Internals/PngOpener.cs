...
{
    internal static PngImage Open(Stream stream)
    {
        if (!stream.CanRead)
        {
        ...
        using (var output = new MemoryStream())
        {
            ...
            return new PngImage(imageHeader, new RawPngData(bytesOut, bytesPerPixel, palette, imageHeader), palette?.HasAlphaValues ?? false);
        }
    }
    ...
    private static bool TryReadChunkHeader(Stream stream, [NotNullWhen(true)] out ChunkHeader? chunkHeader)
    {
        chunkHeader = null;
