...
public record class ChunkHeader(long Position, int Length, string Name)
{
    /// <summary> Whether the chunk is critical (must be read by all readers) or ancillary (may be ignored). </summary>
    public bool IsCritical => char.IsUpper(this.Name[0]);
    ...
    /// </summary>
    public bool IsPublic => char.IsUpper(this.Name[1]);

    /// <summary> Whether the (if unrecognized) chunk is safe to copy. </summary>
    public bool IsSafeToCopy => char.IsUpper(this.Name[3]);
